...
(function(document, $) {
    ...
    $(document).on("foundation-contentloaded", function(e) {
        ...
        $(".cq-dialog-checkbox-showhide").each( function() {
            console.log(" on load function is called");
    ...
    function showHide(el){
        ...
        var classtobefound =el.context.dataset.cqDialogDropdownShowhideTarget;
 console.log("classtobefound"+classtobefound);


        var target = el.data("button-option-enable-showhide-target");
  console.log("target"+target);
        // is checkbox checked?
        var checked = el.prop('checked');
        console.log("checked"+checked);
        if(checked){
			 $(classtobefound).removeClass("hide");
        }else{
			 $(classtobefound).addClass("hide");
        }
